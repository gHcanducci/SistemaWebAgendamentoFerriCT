...
- **Uncommitted (2026-05-29):** remoção de Lista de Espera + Capacidade de turma (decisão de negócio — não cabia no perfil). Models, controllers, views, seed, docs todos limpos. Falta gerar a migration pra dropar as colunas no DB.
- **Próximo passo (obrigatório antes da F5):** abrir Package Manager Console no VS e rodar `Add-Migration RemoveListaEsperaECapacidade`. Isso gera o `DropColumn` automático. Depois F5 aplica via `MigrateDatabaseToLatestVersion`. Sem essa migration, EF dispara `AutomaticMigrationsDisabledException` no startup.
...
**Atualização 2026-05-29:** Lista de Espera + Capacidade de turma foram **removidas** do projeto (regra de negócio descartada — não cabia no perfil da academia). Pendente: rodar `Add-Migration RemoveListaEsperaECapacidade` no PMC pra dropar as colunas. Itens #1, #2, #7, #9, #11 do punch list ficaram obsoletos.

### 🟠 Alto (UX quebrada / ação órfã)
3. **"Registrar Pagamento Manual" inacessível pela UI** — action + view existem mas nem `Agendamentos.cshtml` nem `DetalhesAgendamento.cshtml` linkam. Admin teria que adivinhar URL `/Admin/RegistrarPagamentoManual/{id}`.
...
1. #3 (botão "Registrar Pagamento Manual" em DetalhesAgendamento)
2. #4 (botão "Ver detalhes" no card mobile)
3. #5 (cancelamento pelo cliente)
4. #6 (ownership check em Confirmacao — 1 linha)
5. #8 (whitelist no EditarAgendamento)

#10, #12, #13 são extras se sobrar tempo.
